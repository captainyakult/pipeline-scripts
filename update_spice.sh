...
./dynamogen --update --spice $HOME/sources/spice --output $HOME/sources/dynamo --list $HOME/logs/spice_syncer_updated_spice.log --updatedDynamoFile $HOME/logs/dynamogen_updated_dynamo.log >> $HOME/logs/dynamogen.log 2>&1
popd > /dev/null
rm $HOME/logs/spice_syncer_updated_spice.log

# copy animdata to blackhawk and aws s3
while read folder
do
...
while read folder
do
        $HOME/pipelines/aws_s3_sync/sync.py sync-s3-folder eyes-dev/assets/dynamic/dynamo/$folder $HOME/sources/dynamo/$folder >> $HOME/logs/aws_s3_sync.log 2>&1
        $HOME/pipelines/aws_s3_sync/sync.py sync-s3-folder eyes-staging/assets/dynamic/dynamo/$folder $HOME/sources/dynamo/$folder >> $HOME/logs/aws_s3_sync.log 2>&1
        $HOME/pipelines/aws_s3_sync/sync.py sync-s3-folder eyes-production/assets/dynamic/dynamo/$folder $HOME/sources/dynamo/$folder >> $HOME/logs/aws_s3_sync.log 2>&1
done < $HOME/logs/dynamogen_updated_dynamo.log
rm $HOME/logs/dynamogen_updated_dynamo.log
