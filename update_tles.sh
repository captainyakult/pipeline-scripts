...
for tle_name in $tle_list; do
	...
	if [ -f $BASE/pipelines/animdatagen/config/tles/$pioneer_name.json ] || [ -f $BASE/pipelines/dynamogen/config/tles/$pioneer_name.json ]; then
		# Create the spk.
		echo "    tle -> spk"
		...
		if [ -f $BASE/pipelines/dynamogen/config/tles/$pioneer_name.json ]; then
			echo "    spk -> dynamo"
