...
while read F; do
		...
		cat > $BASE/sources/asteroids/$PIONEER_NAME.earth.orb.json <<- EOM
		{
			"type" : "orb",
			"body" : $NAIF_ID,
			"otherBody" : 399,
		...
		EOM
		# Run dynamo.
		$BASE/pipelines/dynamogen/dynamogen --spice $BASE/sources/spice --output $BASE/sources/dynamo --config $BASE/sources/asteroids/$PIONEER_NAME.earth.orb.json
		rm $BASE/sources/asteroids/$PIONEER_NAME.earth.orb.json
